    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {Object.entries(data.list[0].components).map(([key, components]) => {
                ...
                const max = Math.max(pollutant["Very Poor"].min, components);

                const currentLevel = (() => {
                    for (const [level, range] of Object.entries(pollutant)) {
                        if (components >= range.min && (range.max === null || components <= range.max)) return level;
                    }
                    return 'Very Poor';
                })();
                    ...
                    >
                        ...
                        <div className="flex justify-between">
                            {Object.keys(pollutant).map((quality) => (
                                <span
                                    className={clsx("px-2 py-1 text-xs rounded-md font-medium ",
